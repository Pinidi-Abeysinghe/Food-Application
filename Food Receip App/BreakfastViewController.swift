...
//  Food Receip App
//
//  Created by Sahan Ruwanga on 2023-01-29.
//

import UIKit

class BreakfastViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        
        return allBreakfasts.count
    }
  
    var allBreakfasts: [[String:Any]] = []

    var collectionView = UICollectionView(frame: CGRect(x: 0, y: 120, width: 380, height: 800), collectionViewLayout: UICollectionViewFlowLayout())
    override func viewDidLoad() {
        ...
        getAllBreakfast(completion: )() { (jsonArray) in
            guard let jsonArray = jsonArray else {
                print("No JSONArray received")
                return
            }
            // Use the jsonArray as desired
            self.allBreakfasts = jsonArray
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        self.allBreakfasts = jsonArray.filter { $0.keys.contains("Breakfast") }
            .map { $0["Breakfast"] as! [String : Any] }
        let name = self.allBreakfasts.filter{ $0.keys.contains("Name")}.map{$0["Name"]}
        let image = self.allBreakfasts.filter{ $0.keys.contains("Image")}.map{$0["Image"]}
        let likes = jsonArray.filter { $0.keys.contains("Like") }
            .map { $0["Like"] as! Bool}
        ...
        cell.favButton.tag = (indexPath.item + 1) *  5
        ...
        for (_, element) in indexes.enumerated() {
            if element == cell.favButton.tag/5 - 1  {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        self.allBreakfasts = jsonArray.filter { $0.keys.contains("Breakfast") }.map { $0["Breakfast"] as! [String : Any] }
        let image = self.allBreakfasts.filter{ $0.keys.contains("Image")}.map{$0["Image"]}
        let name = self.allBreakfasts.filter{ $0.keys.contains("Name")}.map{$0["Name"]}
        let link = self.allBreakfasts.filter{ $0.keys.contains("Video")}.map{$0["Video"]}
        let des = self.allBreakfasts.filter{ $0.keys.contains("Details")}.map{$0["Details"]}
